...
This document describes the on-disk layout and file formats for a Collaborative Research Watch workspace as implemented today (Path 1). Product-level entity definitions and data-classification rules remain in `PRD.md`. Path 2 AI record shapes are summarized here where the design is fixed; see `PATH_2_DESIGN.md` for enrichment behavior.

The implementation source of truth for Path 1 record shapes is `backend/src/research_watch/models.py`. Validation and read/write behavior live in `csv_store.py`, `markdown_store.py`, and `sync.py`.
...
| AI output (Path 2) | Same as source ID | `records/ai/<source_id>.md` |
...
- Content changes are detected via file size and modification time (`content_size`, `content_mtime` on the source record).
...
## Path 2: AI Records (Designed, Not Yet Implemented)

AI enrichment is specified in `PATH_2_DESIGN.md`. Storage convention:

```text
records/ai/<source_id>.md
```

Minimum frontmatter (from design; implementation may add fields):
...
The Markdown body contains the AI-generated summary only.
